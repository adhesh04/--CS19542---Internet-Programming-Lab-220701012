...
def admin_dashboard():
    ...
    try:
        query = """
        SELECT id, name, email, designation, branch_office, department, annual_income, points 
        FROM employees
        """
    ...
    return render_template('adminmain.html', employees=employees)
...
def update_points():
    ...
    if action == "add":
        query = "UPDATE employees SET points = points + 1 WHERE id = %s"
